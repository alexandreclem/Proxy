...
int timer_limit; 
int port; // Setting the port as Global, in order to use afterward. Kein problem, because all threds will only cuncur on reading.
...
{    
    // Receiving the Url. //
    char *url = rec_string(consocket); // PARALLELISM PROBLEM HERE. URL NOT SENT -> DESYNCHRONIZATION BETWEEN SOCKETS.: SOLUTION -> usleep(1) BEFORE THE CONNECTION
